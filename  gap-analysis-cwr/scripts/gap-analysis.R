...
narea <- paste(biomod,"/native-areas",sep="")
if (!file.exists(narea)) {dir.create(narea)}

polys <- paste(narea,"/polyshps",sep="")
if (!file.exists(polys)) {dir.create(polys)}

ascis <- paste(narea,"/asciigrids",sep="")
if (!file.exists(ascis)) {dir.create(ascis)}
...
acc <- read.csv(paste(crop_dir,"/biomod_modeling/summary-files/accuracy.csv",sep=""))
...
for (spp in acc$spp) {
  ...
  atauc <- acc$prob.mean.weighted[which(acc$spp==spp)]
  stauc <- acc$sd.prob.mean.weighted[which(acc$spp==spp)]
  ...
  row_res <- data.frame(Taxon=paste(spp),ATAUC=atauc,STAUC=stauc,ValidModel=NA)
...
write.csv(res_all,paste(crop_dir,"/biomod_modeling/summary-files/taxaForRichness.csv",sep=""),quote=F,row.names=F)
...
table_base$ATAUC <- NA; table_base$STAUC <- NA; table_base$IS_VALID <- NA
...
model_met <- read.csv(paste(crop_dir,"/biomod_modeling/summary-files/taxaForRichness.csv",sep=""))
rsize <- read.csv(paste(crop_dir,"/biomod_modeling/summary-files/areas.csv",sep=""))
edist <- read.csv(paste(crop_dir,"/biomod_modeling/summary-files/edist.csv",sep=""))
...
#== calculate final gap richness per genepool ==#
source(paste(src.dir,"/012.gapRichness.R",sep=""))
x <- gapRichness(crop_dir)

#== calculate final gap richness - summary ==#
